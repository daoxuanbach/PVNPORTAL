using System;
using System.ComponentModel;
using System.Web;
using System.Web.UI;
using System.Web.UI.WebControls;
using System.Web.UI.WebControls.WebParts;
using Microsoft.SharePoint;
using Microsoft.SharePoint.WebControls;
using Pvn.Web.Usercontrols;

namespace Internet.Webpart
{
    [ToolboxItemAttribute(false)]
    public class wp_BannerWeb : WebPart
    {
        private const string _ascxPath = @"~/UserControls/ucBannerWeb.ascx";
        ucBannerWeb _uc = new ucBannerWeb();
        protected override void CreateChildControls()
        {
            _uc = (ucBannerWeb)Page.LoadControl(_ascxPath);
            _uc.UrlDetail = this.UrlDetail;
            _uc.SiteENLink = this.SiteENLink;
            _uc.SiteVNLink = this.SiteVNLink;
            _uc.UrlBanner = this.UrlBanner;
            _uc.UrlBackgroundBanner = this.UrlBackgroundBanner;
            this.Controls.Add(_uc);
        }

        private String _urlDetail = "/Pages/newsearch.aspx";
        private string _siteEnLink = "http://english.pvn.vn/";
        private string _siteVNLink = "http://www.pvn.vn/";
        private string _urlBanner = "/Usercontrols/Banner/logoPVN.png";
        private string _urlBackgroundBanner = "/Usercontrols/Banner/background_logo.png";

        [Browsable(false),
       WebDisplayName("UrlBanner"),
       WebBrowsable(true),
       Category("Cấu hình webpart"),
       Personalizable(PersonalizationScope.Shared)]
        public String UrlBanner
        {
            get { return _urlBanner; }
            set { _urlBanner = value; }
        }

         [Browsable(false),
       WebDisplayName("UrlBackgroundBanner"),
       WebBrowsable(true),
       Category("Cấu hình webpart"),
       Personalizable(PersonalizationScope.Shared)]
        public String UrlBackgroundBanner
        {
            get { return _urlBackgroundBanner; }
            set { _urlBackgroundBanner = value; }
        }

        /// <summary>
        /// Url detail
        /// </summary>
        [Browsable(false),
       WebDisplayName("Url detail"),
       WebBrowsable(true),
       Category("Cấu hình webpart"),
       Personalizable(PersonalizationScope.Shared)]
        public String UrlDetail
        {
            get { return _urlDetail; }
            set { _urlDetail = value; }
        }

        /// <summary>
        /// Site EN Link
        /// </summary>
        [Browsable(false),
     WebDisplayName("Site EN Link"),
     WebBrowsable(true),
     Category("Cấu hình webpart"),
     Personalizable(PersonalizationScope.Shared)]
        public String SiteENLink
        {
            get { return _siteEnLink; }
            set { _siteEnLink = value; }
        }

        /// <summary>
        /// Site VN Link
        /// </summary>
        [Browsable(false),
      WebDisplayName("Site VN Link"),
      WebBrowsable(true),
      Category("Cấu hình webpart"),
      Personalizable(PersonalizationScope.Shared)]
        public String SiteVNLink
        {
            get { return _siteVNLink; }
            set { _siteVNLink = value; }
        }
    }
}
