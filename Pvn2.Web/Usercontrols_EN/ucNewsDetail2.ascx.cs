    ...
    public partial class ucNewsDetail2 : System.Web.UI.UserControl
    {
        ...
        private void BindMainNews(DataTable dtMainNews)
        {
            if (dtMainNews != null && dtMainNews.Rows.Count > 0)
            {
                ...
                if (ShowTitle)
                {
                    ltrTitle.Text =  string.Format("<h2 class='post-title bot-m20'><a href ='#'>{0}</a></h2>", Convert.ToString(dtMainNews.Rows[0]["Title"]));
                }
                ...
                {
                    ltrSummary.Text = string.Format("<blockquote>{0}</blockquote>", Convert.ToString(dtMainNews.Rows[0]["Summary"]));
