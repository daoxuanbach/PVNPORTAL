<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="viewJobTitle.aspx.cs" Inherits="AdminLTE.Usercontrols.QLChucDanh.viewJobTitle" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn-add").click(function (event) {
            getPage("/UserControls/Core.Contact/QLChucDanh/fJobTitle.aspx?action=add");
        });
        $('.edit').click(function () {
            var ItemID = $(this).closest("td").attr('data-id')
            getPage("/UserControls/Core.Contact/QLChucDanh/fJobTitle.aspx?action=edit&ItemID=" + ItemID);
        });
    ...
    function fndelete(dataid) {
        $.post(encodeURI("/UserControls/Core.Contact/QLChucDanh/aJobTitle.ashx"), { "PageID": dataid, "hidAction": "del" },
          function (data) {
              if (data.Error) {
                  ...
                  getPage("/UserControls/Core.Contact/QLChucDanh/viewJobTitle.aspx?action=view");
              }
          });
        return false;
    };
    function fnSearch() {
        $.cookie("JobTitle", $('#JobTitle').val(), { path: '/' });
...
<body>
    ...
    <form runat="server" id="frm">
        ...
        <div class="content-header row">
            <div class="col-xs-6">
                <strong class="text-uppercase">DANH SÁCH Chức danh/Titles</strong>
            </div>
            <div class="col-xs-6">
                                ...
                                <div class="col-md-11 vcenter">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label>Tiêu đề</label>
                                                <input id="JobTitle" type="search" name="JobTitle" class="form-control input-sm" placeholder="JobTitle" value="<%=JobTitle%>">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                    ...
                                    <table class="table table-bordered table-striped table-hover">
                                        ...
                                        <asp:Repeater ID="rptDatabind" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    <td class=" actions">
                                                    <input class="checkSingle" type="checkbox" value="<%#Eval("JobTitleID") %>"></td>
                                                    <td><%#(Container.ItemIndex + 1) +  (CurPage-1)*RowPerPage %></td>
                                                    <td><%#Eval("JobTitle") %></td>
                                                    <td><%#Eval("UsedStateName") %></td>
                                                    <td class=" actions" data-id="<%#Eval("JobTitleID") %>">
                                                        <%=ucChucNang.ChucNang %>
                                                    </td>
                                                 </tr>
                                            </ItemTemplate>
