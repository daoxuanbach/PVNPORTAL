<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="viewLoaiLienHe.aspx.cs" Inherits="AdminLTE.Usercontrols.LoaiLienHe.viewLoaiLienHe" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn-add").click(function (event) {
            getPage("/UserControls/Core.Contact/LoaiLienHe/fLoaiLienHe.aspx?action=add");
        });
        $('.edit').click(function () {
            var ItemID = $(this).closest("td").attr('data-id')
            getPage("/UserControls/Core.Contact/LoaiLienHe/fLoaiLienHe.aspx?action=edit&ItemID=" + ItemID);
        });
    ...
    function fndelete(dataid) {
        $.post(encodeURI("/UserControls/Core.Contact/LoaiLienHe/aLoaiLienHe.ashx"), { "PageID": dataid, "hidAction": "del" },
          function (data) {
              if (data.Error) {
    ...
    function fnSearch() {
        $.cookie("ContactType", $('#ContactType').val(), { path: '/' });
...
<body>
    ...
    <form runat="server" id="frm">
        ...
        <div class="content-header row">
            <div class="col-xs-6">
                <strong class="text-uppercase">DANH SÁCH CONTACTTYPE</strong>
            </div>
            <div class="col-xs-6">
                                ...
                                <div class="col-md-11 vcenter">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label>Tiêu đề</label>
                                                <input id="ContactType" type="search" name="ContactType" class="form-control input-sm" placeholder="ContactType" value="<%=ContactType%>">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                    ...
                                    <table class="table table-bordered table-striped table-hover">
                                        ...
                                        <asp:Repeater ID="rptDatabind" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    <td class=" actions">
                                                    <input class="checkSingle" type="checkbox" value="<%#Eval("ContactTypeID") %>"></td>
                                                    <td><%#(Container.ItemIndex + 1) +  (CurPage-1)*RowPerPage %></td>
                                                    <td><%#Eval("ContactType") %></td>
                                                    <td><%#Eval("UsedStateName") %></td>
                                                    <td class=" actions" data-id="<%#Eval("ContactTypeID") %>">
                                                        <%=ucChucNang.ChucNang %>
                                                    </td>
                                                 </tr>
                                            </ItemTemplate>
