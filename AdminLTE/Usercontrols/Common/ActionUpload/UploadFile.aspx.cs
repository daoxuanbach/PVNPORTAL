...
namespace AdminLTE.Usercontrols.Common.ActionUpload
{
    public partial class UploadFile : System.Web.UI.Page
    {
        ...
        private string uploadFile(Stream serverFileStream, string fileName)
        {
            ...
            path = Server.MapPath("/UserControls/Upload/File/") + fileName;
            if (this.ListAllImageAllow().Contains(extension))
            {
                path = Server.MapPath("/UserControls/Upload/Images/") + fileName;
            }
            try
            {
                int count = 0x100;
