<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="viewImageCategoryList.aspx.cs" Inherits="AdminLTE.Usercontrols.ImageCategoryList.viewImageCategoryList" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn-add").click(function (event) {
            getPage("/UserControls/ImageCategoryList/fImageCategoryList.aspx?action=add");
        });
        $('.edit').click(function () {
            var ItemID = $(this).closest("td").attr('data-id')
            getPage("/UserControls/ImageCategoryList/fImageCategoryList.aspx?action=edit&ItemID=" + ItemID);
        });
    ...
    function fndelete(dataid) {
        $.post(encodeURI("/UserControls/ImageCategoryList/aImageCategoryList.ashx"), { "CategoryID": dataid, "hidAction": "del" },
            function (data) {
                if (data.Error) {
    ...
    function fnSearch() {
        ...
        postData(1, "/UserControls/ImageCategoryList/viewImageCategoryList.aspx?Language=" + Language + "&UsedState=" + UsedState);
    }
...
<body>
    ...
    <form runat="server" id="frm">
        <div class="content-header row">
            <div class="col-xs-6">
                <strong class="text-uppercase">DANH SÁCH CHUYÊN MỤC ẢNH </strong>
            </div>
            <div class="col-xs-6">
                                    ...
                                    <table class="table table-bordered table-striped table-hover">
                                        ...
                                        <asp:Repeater ID="rptDatabind" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    <td class=" actions">
                                                        <input class="checkSingle" type="checkbox" value="<%#Eval("ImageCategoryID") %>"></td>
                                                    <td><%#Eval("OrdinalTitle") %></td>
                                                    <td><%#Eval("Code") %></td>
                                                    <td><%#Eval("Ordinal") %></td>
                                                    <td><%#Eval("ImageCategoryID") %></td>
                                                    <td><%#Eval("UsedStateName") %></td>
                                                    <%--<td><%#Eval("LanguageName") %></td>--%>
                                                    <td class="actions" data-id="<%#Eval("ImageCategoryID") %>">
                                                        <%=ucChucNang.ChucNang %>
                                                        <%--<a class="edit" href="javascript:void(0)" data-id="<%#Eval("ImageCategoryID") %>" title="Xóa"><i class="fa fa-edit (alias)"></i></a>--%>

                                                    </td>
                                                    <%--     <td class=" actions"><a class="delete" href="javascript:void(0)" data-id="<%#Eval("ImageCategoryID") %>" title="Xóa"><i class="fa fa-trash-o"></i></a></td>--%>
                                                </tr>
                                            </ItemTemplate>
                                        </asp:Repeater>
