<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="viewMeeting.aspx.cs" Inherits="AdminLTE.Usercontrols.Meeting.viewMeeting" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn-add").click(function (event) {
            getPage("/UserControls/Core.Meeting/Meeting/fMeeting.aspx?action=add");
        });
        $('.edit').click(function () {
            var ItemID = $(this).closest("td").attr('data-id')
            getPage("/UserControls/Core.Meeting/Meeting/fMeeting.aspx?action=edit&ItemID=" + ItemID);
        });
    ...
    function fndelete(dataid) {
        $.post(encodeURI("/UserControls/Core.Meeting/Meeting/aMeeting.ashx"), { "PageID": dataid, "hidAction": "del" },
            function (data) {
                if (data.Error) {
...
<body>
    ...
    <form runat="server" id="frm">
        ...
        <div class="content-header row">
            <div class="col-xs-6">
                <strong class="text-uppercase">Danh sách Meeting</strong>
            </div>
            <div class="col-xs-6">
                                ...
                                <div class="col-md-11 vcenter">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label>Tiêu đề</label>
                                                <input id="Title" type="search" name="Title" class="form-control input-sm" placeholder="Title" value="<%=Title%>">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                ...
                                <table class="table table-bordered table-striped table-hover">
                                    <thead>
                                        <tr class="table-header">
                                            ...
                                            <th class="col-xs-2">Ngày họp</th>
                                            <th class="col-xs-3">Phòng họp</th>
                                    ...
                                    <asp:Repeater ID="rptDatabind" runat="server">
                                        <ItemTemplate>
                                            <tr>
                                                <td class=" actions">
                                                    <input class="checkSingle" type="checkbox" value="<%#Eval("MeetingID") %>"></td>
                                                <td><%#(Container.ItemIndex + 1) +  (CurPage-1)*RowPerPage %></td>
                                                <td><%#string.Format("{0:dd/MM/yy} {0:T}", Eval("MeetingDate"))%></td>
                                                <td><%#Eval("RoomName") %></td>
                                                <td><%#Eval("Title") %></td>
                                                <td><%#Eval("Note") %></td>
                                                <td class=" actions" data-id="<%#Eval("MeetingID") %>">
                                                    <%=ucChucNang.ChucNang %>
                                                </td>
                                            </tr>
                                        </ItemTemplate>
