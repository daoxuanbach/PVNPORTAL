<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="viewCompanyChart.aspx.cs" Inherits="AdminLTE.Usercontrols.CompanyChart.viewCompanyChart" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn-add").click(function (event) {
            getPage("/UserControls/CompanyChart/fCompanyChart.aspx?action=add");
        });
        $('.edit').click(function () {
            var ItemID = $(this).closest("td").attr('data-id')
            getPage("/UserControls/CompanyChart/fCompanyChart.aspx?action=edit&ItemID=" + ItemID);
        });
        ...
        $("#cboCompanyType").select2("val", "<%=CompanyType%>");

    })
    function fndelete(dataid) {
        $.post(encodeURI("/UserControls/CompanyChart/aCompanyChart.ashx"), { "PageID": dataid, "hidAction": "del" },
          function (data) {
              if (data.Error) {
    ...
    function fnSearch() {
        ...
        $.cookie("CompanyType", $('#cboCompanyType').val(), { path: '/' });
                                    ...
                                    <table class="table table-bordered table-striped table-hover">
                                        ...
                                        <asp:Repeater ID="rptDatabind" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    <td class=" actions">
                                                    <input class="checkSingle" type="checkbox" value="<%#Eval("CompanyChartID") %>"></td>
                                                    <td><%#(Container.ItemIndex + 1) +  (CurPage-1)*RowPerPage %></td>
                                                     <td><%#Eval("CompanyTitle") %></td>
                                                    <td><%#Eval("CompanyTypeName") %></td>
                                                    <td><%#Eval("Ordinal") %></td>
                                                      <td><%#Eval("UsedStateName") %></td>
                                                    <td class=" actions" data-id="<%#Eval("CompanyChartID") %>">
                                                        <%=ucChucNang.ChucNang %>
                                                    </td>
                                                 </tr>
                                            </ItemTemplate>
